...
/**
 * Test-only provider for the [OAuthClientMetadataUrl]-qualified URL
 * that `:core:auth`'s `AtOAuthModule.provideAtOAuth` requires. The
 * production binding lives in `:app/src/main/.../OAuthConfigModule.kt`
 * and reads `BuildConfig.OAUTH_CLIENT_METADATA_URL`, neither of which
 * is on the classpath when running `:feature:composer:impl/src/androidTest/`.
 *
 * Returns a placeholder URL — the composer dialog instrumentation test
