 ...
 * the live width-class signal and the live state holders into it.
 *
 * Re-keys on width-class so a foldable unfold or device rotation
...
internal fun rememberComposerLauncher(
    ...
    return remember(widthSizeClass, navState, launcherState) {
        { replyToUri: String? ->
            launchComposer(
 ...
 * launcher path fires based on [widthSizeClass]:
 *
 * - [WindowWidthSizeClass.COMPACT] → [onPushRoute]
 * - anything else (Medium / Expanded) → [onShowOverlay]
 *
 ...
 * follow-up. As long as `currentWindowAdaptiveInfo()` reports the
