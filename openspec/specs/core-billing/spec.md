# core-billing Specification

## Purpose
The SDK-agnostic billing boundary (`:core:billing`) behind Nubecita Pro. `EntitlementRepository` exposes entitlement state (`isPro`, `activeSubscription`); `BillingRepository` initiates offering load, purchase, and restore. No provider SDK type leaks past this module, and identity is the anonymous Google Play `appUserID` — never the Bluesky DID.
## Requirements
