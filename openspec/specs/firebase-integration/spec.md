...
## Purpose
Firebase bootstrap from committed configuration: App Check provider factory installation and Analytics SDK availability, plus the release plumbing that ties `versionName` to the semantic-release version and distributes the debug APK to App Distribution on publish. Service-account credentials are never committed, and an instrumented test verifies the wiring.
## Requirements
