# dm-background-notifications Specification

## Purpose
Notify a signed-in user of new inbound direct messages while the app is backgrounded, **credential-free and gateway-free** — the device polls its own inbox with the session it already holds (via `:core:auth`) and posts **local** notifications. Real FCM push is deferred (`nubecita-1fy.13`) because Bluesky DMs are private (`chat.bsky.convo`, not on Jetstream), so a server would have to custody full-account credentials per user. The non-negotiable constraint is battery: stay within Google's WorkManager guidelines and cooperate with Doze / App Standby rather than fight them — the accepted UX bar is "best-effort, may be delayed." A single user-facing toggle gates this background poller **and** v1's foreground unread poller together (design D6). Implemented in `nubecita-1fy.15`.
