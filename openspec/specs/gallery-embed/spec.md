# gallery-embed Specification

## Purpose
End-to-end support for Bluesky's `app.bsky.embed.gallery` multi-image embed (soft limit 10, distinct from the 4-image `app.bsky.embed.images`): mapping the gallery view union to a UI model, rendering it in the post-card carousel and the full-screen lightbox, and authoring galleries in the composer — a 10-image picker with drag-reorder, count-based emission (images for ≤4 / gallery for ≥5), per-image alt text (with a gallery required-alt gate), and aspect ratio.
