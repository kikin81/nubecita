...
## Purpose
Verification badges derived from atproto's verification state — rendered for feed post authors and on the profile header, with a tappable bottom sheet naming the verifiers and an accessible presentation. Verification data is advisory and independent of any Nubecita-issued status.
## Requirements
