# feature-notifications Specification

## Purpose
The in-app notifications surface (`:feature:notifications:{api,impl}`): a `@MainShell`-qualified Nav3 tab over `listNotifications`, following the project's MVI conventions with flat state plus a sealed `NotificationsLoadStatus`. Covers `paginate`-based pagination, filter chips mapped to the lexicon's `reasons[]`, client-side single/multi-actor aggregation, and batched `getPosts` subject hydration.
## Requirements
