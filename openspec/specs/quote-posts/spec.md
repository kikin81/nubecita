# quote-posts Specification

## Purpose
Define how Nubecita authors and honors quote posts (`app.bsky.embed.record` /
`app.bsky.embed.recordWithMedia`). Quoting reuses the single unified composer —
reply target and quote target are independent optional inputs — and is reachable
from the repost affordance's menu and from pasting a Bluesky post link while
composing. On the read side, the app derives `canViewerQuote` from the wire
`viewer.embeddingDisabled` and refuses to quote posts whose authors have disabled
quoting. Embed construction is centralized in `:core:posting` behind one
intent-to-embed resolver so new embed types never require a second composer.
