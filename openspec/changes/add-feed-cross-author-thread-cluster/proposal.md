...
- **`FeedViewPostMapper.toFeedItemUiOrNull(...)`** — replaces the existing `toPostUiOrNull(...)` as the entry-point mapper. Detects a populated `reply: ReplyRef` with renderable `parent` + `root` and produces `ReplyCluster`; otherwise produces `Single`. `BlockedPost` / `NotFoundPost` parent variants fall back to `Single` for v1 — context is dropped silently rather than rendering "Replying to a hidden post" (deferred to a follow-up). The existing `toPostUiOrNull(...)` private helper stays internal as the leaf-projection step.
