...
- [ ] 4.1 `./gradlew :designsystem:updateDebugScreenshotTest` (241 baselines) and `:core:image` (2). **`:app` is FLAVORED** and needs `:app:updateProductionDebugScreenshotTest` (22) — it declares the flavor dimension through its own convention plugin, so `git grep nubecita.android.flavors` does NOT match `app/build.gradle.kts`. That grep, which CLAUDE.md gives as the flavored-module test, has a blind spot for exactly `:app` and `:benchmark`. Validate every task path with `--dry-run` before the real run; an ambiguous name fails the whole invocation after configuration.
