...
- [ ] 2.3 Change `feature/onboarding/impl/.../OnboardingScreen.kt:309` to DROP the tint override entirely and take `NubecitaLogomark`'s default of `MaterialTheme.colorScheme.primary`. Unlike the splash placeholder this is in-app chrome shown after the handoff, so it should follow the active theme — including wallpaper-derived color under `AppTheme.Dynamic`, which the pinned constant defeats. Remove the now-unused `NubecitaPalette` import.
