...
- [ ] 1.6 Implement `internal/DefaultImageSaver`: read the Coil disk-cache snapshot for the URL; on a miss, execute the `ImageRequest` and re-read; stream the bytes into a `MediaStore.Images` row with `RELATIVE_PATH = "Pictures/Nubecita/"` and `IS_PENDING = 1`, clearing the flag only after a complete write. Write the trailing slash: `MediaProvider` normalises either form on insert, but the trailing slash is the canonical *persisted* shape (`RELATIVE_PATH`'s javadoc gives `DCIM/Vacation/` for a file in `DCIM/Vacation`), so writing it that way makes the value read back identical to the value written and keeps 5.2's assertion exact. Write `altText` to `DESCRIPTION` when present. Run on `@IoDispatcher`. **Do not decode/re-encode.**
- [ ] 1.7 Delete the inserted row if the write does not complete — on exception **and on cancellation**. A pending row is invisible to the gallery but still consumes storage the user cannot reclaim, so the insert needs `try`/`catch`/`finally` ownership, not just the `IS_PENDING` flag. Note `withContext` cancellation will not run suspending cleanup, so the delete must be non-suspending or wrapped with `NonCancellable`.
...
- [ ] 1.9 Implement `isSupported` as the API-29+ check, with a comment recording the store-listing rationale and the 1.0% measurement from `design.md` D3 — so a future reader does not "fix" it by adding the permission.
- [ ] 1.10 Bind `ImageSaver` → `DefaultImageSaver` in the existing `ImageModule`.
...
- [ ] 4.4 Suspend the chrome auto-fade timer while `isSaving` is true, and resume it on completion. The progress indication lives in the chrome, so without this the timer hides the very feedback the spec requires be visible.
- [ ] 4.5 Add the new strings to `values/strings.xml` **and** to `values-b+es+419/` and `values-pt-rBR/` in the same commit, then run this module's own lint (`:app` lint does not catch `MissingTranslation` here). Note the Spanish qualifier is the BCP-47 form `values-b+es+419`, **not** `values-es-r419`.

## 5. Tests

- [ ] 5.1 JVM unit tests for the content-type sniffer (JPEG/PNG/WebP plus the unrecognised fallback) and for filename generation.
- [ ] 5.2 Instrumented `androidTest` for `DefaultImageSaver`, mirroring `:core:posting`'s `DefaultSharedMediaStoreTest`: asserts the row's `RELATIVE_PATH` reads back as exactly `Pictures/Nubecita/`, that saved bytes are byte-identical to the source, that the recorded content type matches the sniffed type, and that `altText` reaches `DESCRIPTION`.
...
- [ ] 5.4 Instrumented coverage for cleanup: a save that fails mid-write leaves **no** row behind — not merely no *visible* row. Query including pending entries, so a leaked `IS_PENDING` row fails the test rather than passing as "invisible".
- [ ] 5.5 ViewModel tests with a fake `ImageSaver`: `isSaving` toggles and clears on both outcomes; a second `OnSaveClick` during a save starts no second save; the correct outcome effect is emitted per `Result`; `canSave` is false when the fake reports unsupported.
- [ ] 5.6 Verify the tests discriminate — mutate the current-page selection to always save index 0 and confirm a test fails. Both modules are **unflavored**: use plain `testDebugUnitTest` / `lintDebug`.
