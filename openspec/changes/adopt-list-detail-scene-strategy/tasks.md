...
- [ ] 4.3 Add a scenario to `app/src/androidTest/java/net/kikin/nubecita/shell/MainShellPersistenceTest.kt`: place `[Feed]` on the back stack at medium width, rotate to compact, rotate back to medium — assert the `FeedDetailPlaceholder` is visible in the right pane after the round-trip.
- [ ] 4.4 Run `./gradlew :app:validateDebugScreenshotTest :feature:feed:impl:validateDebugScreenshotTest` — diffs reviewed and committed.
- [ ] 4.5 Run `./gradlew testDebugUnitTest` repo-wide — green.
