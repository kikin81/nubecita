...
- [ ] 3.1 Limits probe: call `getUploadLimits` against a `video.bsky.app`-scoped `XrpcClient` built with an `AuthProvider` that resolves `Authorization: Bearer <serviceAuthToken>` per request — the endpoint is documented "for the authenticated user", and a lazily-resolved provider is needed because the JWT expires in 30 min and is minted on demand. The singleton `HttpClient` IS safe to reuse: it carries only timeouts and logging, since auth in this SDK lives on the `XrpcClient`, not the transport. Map `canUpload == false` to `Failed(NotPermitted(message))`. Test: `VideoUploadLimitsTest` with Ktor `MockEngine` — permitted proceeds, rejected terminates and never invokes the transcoder (assert via a spy transcoder).
