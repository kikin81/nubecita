...
- [ ] B3 Manifest (`:app`): add `ACTION_SEND` intent-filters to `MainActivity` for `text/plain` and `image/*` (single). `category.DEFAULT`. Ship the empty `consumer-rules.pro` rule only if a new module is added (none here).
- [ ] B4 `MainActivity.handleIntent`: new `ACTION_SEND`/`ACTION_SEND_MULTIPLE`(reject multiple in v1) branch **before** the `uri == null` FCM early-return. Parse via `ShareIntentParser`; on Text/Url → `deepLinkRouter.publish(ComposerRoute(sharedText = …))`; then **strip the share extras** (`setIntent(Intent())` or remove `EXTRA_TEXT`/`EXTRA_STREAM`). Re-run the same branch from `onNewIntent`.
...
- [ ] B6 `:app:assembleProductionDebug` links; unit tests green. Bench smoke: `adb shell am start -a android.intent.action.SEND --es android.intent.extra.TEXT "https://example.com" -t text/plain net.kikin.nubecita` → composer opens carded.
