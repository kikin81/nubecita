...
- [ ] 2.1 Remove the unconditional `@Binds WidgetUpdater` from `:core:widget-sync`'s `WidgetSyncModule`; keep `NoOpWidgetUpdater` as a bindable default. Re-run `:core:widget-sync` JVM tests (they inject fakes directly — must stay green).
- [ ] 2.2 Bind `WidgetUpdater` per `:app` flavor: production → (placeholder, real impl lands in 5.x) ; bench/non-production → `NoOpWidgetUpdater`. Verify both `:app:assembleProductionDebug` and `:app:assembleBenchDebug` assemble (no Hilt duplicate/missing-binding error).

## 3. Image-prefetch seam in the worker (D-C4) — modifies widget-feed-refresh

- [ ] 3.1 Add `interface WidgetImagePrefetcher { suspend fun prefetch(feedKey: FeedKey) }` + `NoOpWidgetImagePrefetcher` to `:core:widget-sync`; bind the no-op as the default (flavor-overridable like the updater). No `androidx.glance` dependency added to `:core:widget-sync`.
- [ ] 3.2 Call `prefetch(feedKey)` from `WidgetRefreshRunner` after a feed's `trimToCap` succeeds, inside the same per-feed try/catch (rethrow `CancellationException`; a prefetch throw fails only that feed's images, never the refresh/updater/other feeds). Unit tests (extend `WidgetRefreshRunnerTest`): prefetch invoked once per succeeded feed; a throwing prefetch leaves the worker `SUCCESS` and the other feed still refreshed+updated.
