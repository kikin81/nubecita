...
### D5: aspectRatio computed at pick time, carried on the attachment
`aspectRatio` is computed once at pick/attach time via a bounds-only decode (`BitmapFactory.Options.inJustDecodeBounds = true`) of the original image, off the main thread, and carried as width/height on `PickedImage` → `ComposerAttachment`. Posting reads it onto `GalleryImage` (required) and also onto `ImagesImage` (currently `AtField.Missing`) as a free feed-layout improvement.
- *Why this, not "decode in `uploadOne`":* the earlier draft assumed the bitmap is already decoded in `uploadOne` and bounds were free. That is false — `uploadOne` delegates encoding to `ImageEncoder.encodeForUpload` in `:core:image`, whose `EncodedImage` returns only `bytes`+`mimeType`, and which is **pass-through (no decode) for images ≤ 1 MB**. Computing in `uploadOne` would therefore force a second decode and/or widen `EncodedImage`. A bounds-only decode at pick time always runs, allocates no bitmap, and is **scale-invariant** — the WebP re-encode preserves aspect ratio, so measuring the original is correct regardless of any downscale. The picker already opens the `Uri` to read its MIME type, so the bounds decode rides along there.
- *Impact:* `:core:image` (`PickedImage` gains dimensions) and `:core:posting` (`ComposerAttachment` gains `aspectRatio`). Both are listed in the proposal's Impact section.
