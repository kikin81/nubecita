...
- [ ] 1.3 `CardyBExternalLinkMetadataRepository` impl using the singleton Ktor `HttpClient` from `:core:auth` DI; short timeout; **URL-encode** the `url` param; `null` only on `error`/network/timeout/no-title; blank description allowed; default missing title/description to `""`.
...
- [ ] 1.6 Unit tests with Ktor `MockEngine`: success → `LinkPreview`; `error`/no-title → null; blank description → card with `""`; network fail → null; `downloadThumb` success / oversize-rejected / fail.
