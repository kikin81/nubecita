...
Bluesky's feed lexicon `app.bsky.embed.video#view` carries an HLS playlist URL plus an optional JPEG/WebP poster (`thumbnail`), an optional aspect ratio, an optional alt-text, and a `presentation` hint. **The lexicon does NOT currently expose a duration field** (verified against the upstream Bluesky lexicon at `app/bsky/embed/video.json` and the `atproto-kotlin` 5.1.2 generated `VideoView` data class — only `cid`, `playlist`, `thumbnail`, `aspectRatio`, `presentation`, `alt` are present). The original draft of this design assumed a "duration in milliseconds" field; that assumption was wrong. The PDS / AppView serves the .m3u8 via the `https://video.bsky.app/...` CDN with adaptive bitrate variants (typically 360p / 480p / 720p). A typical Following timeline page contains 0–5 video posts.

`EmbedUi.Video.durationSeconds` is therefore declared as `Int?` and the v1 mapper passes `null` for every video post. The render layer renders the duration chip ONLY when `durationSeconds != null`; v1 effectively ships without a duration chip on any video card. The field is reserved for a future phase that sources duration either from a lexicon evolution (Bluesky adds the field) or from HLS manifest parsing after the player loads (`EXT-X-PLAYLIST-TYPE: VOD` segment-duration sum) — the latter is phase-C-or-later work since the manifest isn't loaded until the coordinator binds the player.
