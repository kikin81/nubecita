...
- **MODIFIED capability** `data-models` — adds `EmbedUi.Video` variant to the sealed `EmbedUi` interface (poster URL, HLS URL, aspect ratio, duration, alt text). `FeedViewPostMapper.toEmbedUi` adds the dispatch arm.
