...
- [ ] 2.7 Add `WindowSizeClass` evaluation at screen root; render Compact = `Scaffold` + `TopAppBar` (back arrow), Medium/Expanded = modal `Surface` over scrim, ≤640dp max width, ≤80% window height max, rounded corners, X-close in top-trailing
...
- [ ] 2.10 `@Preview` + `@PreviewTest` for phone shape (compact) and tablet shape (modal) — uses `@PreviewNubecitaScreenPreviews` per memory; both signed-in and missing-display-name fixtures
