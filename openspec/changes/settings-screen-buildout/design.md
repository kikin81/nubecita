...
- atproto-kotlin gaps: as of this change, the SDK's notification lexicon exposes only `getUnreadCount`, `listNotifications`, and `updateSeen` (`putPreferences` is missing); the chat lexicon similarly omits `getConvoForMembers` and convo-lifecycle ops. Settings tasks that touch server-side preferences MUST verify lexicon coverage in the generated atproto-kotlin sources before scoping, and file upstream issues at `kikin81/atproto-kotlin` when helpers are missing.
- UI test coverage convention: every UI-touching task in this repo ships unit tests + `@Preview` + screenshot tests; tasks that also include android-instrumentation tests need the `run-instrumented` GitHub label on their PR so the CI's instrumented job actually runs.
...
The screen Composable resolves the current window-size class once at the screen root via `currentWindowAdaptiveInfoV2().windowSizeClass.isWidthAtLeastBreakpoint(WindowSizeClass.WIDTH_DP_MEDIUM_LOWER_BOUND)` — the project-wide adaptive pattern (see `app/.../shell/MainShell.kt`, `app/.../shell/composer/RememberComposerLauncher.kt`, and `feature/composer/impl/.../internal/ComposerDiscardDialog.kt` for existing call sites). The `WindowSizeClass` type comes from `androidx.window.core.layout`, NOT the older material3 window-size-class enums (`WindowWidthSizeClass.COMPACT/MEDIUM/EXPANDED` don't exist on this codebase's adaptive deps). The resolved boolean chooses between two wrapper Composables:

- **Below Medium breakpoint (phone)**: existing `Scaffold` + `TopAppBar` with back-arrow; content fills the screen.
- **At or above Medium breakpoint (tablet, foldable, desktop)**: `Dialog` (or a custom modal `Surface` over a scrim) with an `IconButton(X)` in the top-trailing slot, fixed `maxWidth ≈ 640dp`, `maxHeight ≈ 80% of window height` (clamped so the modal never reaches the system bars on tall tablets and never clips on landscape foldables), centered.
...
- **`atproto-kotlin` lexicon gaps block server-preference tasks.** Notifications `putPreferences` is missing in the current SDK; content-warning `putPreferences` coverage needs verification. → Mitigation: each affected task starts with a lexicon check against the atproto-kotlin SDK's generated lexicon sources (the SDK is sibling to nubecita; readers verify by inspecting their local `kikin81/atproto-kotlin` checkout or the published `io.github.kikin81.atproto:*` artifact sources). File upstream issues with `gh issue create --repo kikin81/atproto-kotlin --title "<lexicon op>"` when helpers are missing. Tasks split into "lexicon work" + "UI wiring" subtasks when needed.
...
- **Coil isn't wired yet.** Image-quality picker has no consumer until Coil lands. → Mitigation: ship the row as a UI-only preference (persists, no effect). The row's `@Preview` + screenshot tests use the same stubbed-fixture approach as every other Settings row — no image loading required. When Coil lands, a follow-up wires the policy through.
...
- **The shell rebuild changes `SettingsStubContract` shape.** Existing screenshot + instrumentation tests need rewriting. → Mitigation: treat the shell rebuild as a "replace and verify" task — drop the old screenshot baselines, add new ones for the rebuilt shell. The PR carries the `update-baselines` GitHub label so the screenshot-baseline CI job regenerates the committed baselines from the new fixtures.
