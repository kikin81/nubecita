...
- Remove the `BLUESKY_SIGNUP_URL` constant from `LoginViewModel.kt` (no longer referenced; instrumentation test reads the URL contract from the live PAR'd response).
- Tests:
  ...
  - Instrumentation: update the existing CTA-tap test to assert the intent's URI host equals `bsky.social` and the path starts with `/oauth/authorize`, instead of an exact-URL match against `https://bsky.app/`. The `request_uri` query param is dynamic per PAR call.
