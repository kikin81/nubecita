...
`isEligible = posts ≥ 3 ∧ (now − firstInstallTime) ≥ 3d ∧ requestCount < 3 ∧ (lastRequested == null ∨ (now − lastRequested) ≥ 90d)`. The predicate is a pure function over a `ReviewState` snapshot (DataStore counters), the OS-provided first-install time, and `now` (from `:core:common`'s clock), so the full truth table is unit-testable with zero IO. `firstInstallTime` comes from `PackageManager.getPackageInfo(...).firstInstallTime` (adopted from review feedback) rather than a self-stamped `firstLaunchAt`: it reflects long-time users correctly on upgrade (no artificial 3-day delay), survives app-data clears, and needs no startup initializer. Day math uses **exact `Duration`**, not calendar days, to stay timezone/region-safe. *Alternative:* stamp our own `firstLaunchAt` in an `AppInitializer` — delays existing users on upgrade and adds a startup hook; calendar-day math (fragile across timezones).
...
`src/production/ReviewModule` binds `DefaultReviewManager` + `PlayReviewClient` + `DefaultInstallTimeProvider`; `src/bench/ReviewModule` binds a no-op `BenchFakeReviewManager`. The first-install time is read on-demand from `PackageManager` (no startup initializer). Bench builds bind the no-op manager, which never reads install time or calls Play. Result: macrobench/keyless builds make zero Play calls and never prompt — same guarantee billing gives.
