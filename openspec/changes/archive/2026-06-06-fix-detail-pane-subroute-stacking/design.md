...
- *Per-tab `sceneKey` on every list/detail metadata* (the Material3-native way to host multiple list-detail scaffolds in one `NavDisplay`): would scope panes to a tab without the wrapper. **Rejected** — it can't tag **shared sub-routes**: a `PostDetail`/author-`Profile` is pushed onto whichever tab is active, and its metadata is derived from the route instance, which carries no host-tab, so it can't be given the host tab's `sceneKey`. Positional active-tab slicing is host-tab-agnostic and handles shared sub-routes; the wrapper fills a genuine gap. (Confirmed by a nav3-best-practices audit of the implementation.)
