...
4. **`ComposerViewModel` constructor seam.** The V1 constructor signature is `class ComposerViewModel @AssistedInject constructor(@Assisted route: ComposerRoute, postingRepository: PostingRepository, parentFetchSource: ParentFetchSource)` with `@HiltViewModel(assistedFactory = ComposerViewModel.Factory::class)`. Adding `draftRepository: DraftRepository` later is a one-line **appended** parameter — no other constructor parameter or call-site shift. The contract is append-only. Hilt resolves new dependencies automatically; the assisted-injection pattern matches `:feature:postdetail:impl`'s `PostDetailViewModel` precedent.
