...
- **[spike] Existing baselines DO move — 5 of them, legitimately.** The original prediction ("every fixture renders at scroll 0 → identical 'Post' Text → baselines don't move") was wrong. Swapping the inline `TopAppBar { Text }` for the `AnimatedContent`-wrapped title measures the title a hair differently, which sub-pixel-shifts the Scaffold body; the title text re-snaps to the same pixels, but the reply FAB's soft drop-shadow reveals the shift (~193 px, above the AGP comparison threshold). Confirmed it is the toolbar swap and not the later edits by validating the WIP commit's clean tree — the same 5 focus-bearing light fixtures fail there. The 5 updated baselines (`single-post`, `loaded`, `loaded-refreshing`, `container-hierarchy`, `multi-image-carousel`) are correct and must be regenerated + committed with the change. The diff is imperceptible and unavoidable (the swap *requires* `AnimatedContent`). Dark variants are unaffected (shadow contrast differs), which is why only the light ones move.
