...
**Goals**
- A `:core:feed-cache` module owning feed fetch, mapping, and an offline cache — the single source of truth, depended on by both the app and (later) the widget worker.
...
### D-A1. New `:core:feed-cache` module (not `:core:posts`, not in-feature)
A widget cannot depend on a feature `:impl`, and `:core:posts` is single-post/thread. A dedicated `:core:feed-cache` (`nubecita.android.library` + `nubecita.android.hilt`) is the clean shared home. Depends on `:core:database`, `:core:auth` (session DID + `XrpcClientProvider`), `:core:feed-mapping` (reuse existing wire→`PostUi` mappers), `:data:models`, `atproto`. *Alternative — grow `:core:posts`:* conflates single-post and feed-list concerns; rejected.
...
`FeedPostEntity`/`FeedRemoteKeyEntity` never leave `:core:feed-cache`; `fun FeedPostEntity.asExternalModel(): PostUi` (same-file extension) returns a `:data:models` type. Reads return `Flow`/`PagingData`; writes are `suspend`; multi-statement writes are `@Transaction`. Mirrors `:core:database` conventions.

### D-A7. Saved-feeds fetch
A `SavedFeedsRepository`-style accessor in `:core:feed-cache` (`app.bsky.actor.getPreferences` for the saved/pinned list + feed-generator display metadata) to back the Pro picker in C/D. No cache in A (one-shot fetch); revisit if C needs offline. May reuse existing custom-feeds work (`nubecita-lq9t.3.2`) / feed-switching (`nubecita-a580`).
...
1. Land `:core:feed-cache` with the cache, mediator, head query, and saved-feeds fetch behind a repository interface — **no consumer rewired yet** (additive; `feature/feed` keeps working via its current path or a thin delegation).
