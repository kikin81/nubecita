...
- [x] 3.4d **Activity-recreation survival (design D5).** Rotated with `Light` forced and the OS dark: the app stays light, with no reset to `null` and no re-gate — the `@Singleton` outlives the Activity. Rotation is the same recreation path as folding; **no foldable was available** (none attached, no foldable AVD), so the fold-specific gesture itself is still unverified.
