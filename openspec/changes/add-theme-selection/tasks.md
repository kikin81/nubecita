...
- [ ] 3.4b **Device pass, forced-theme half** — deferred to task 4.7, not skipped. `Dark`-on-light-OS and `Light`-on-dark-OS cannot be exercised at this layer: there is no picker yet, and the bench flavor's `FakeUserPreferencesRepository` hard-codes `DYNAMIC`, so no code path can select another theme. Verify both once the Appearance screen lands, confirming (a) the first content frame after a force-stop + cold start is the chosen theme with no flash and (b) status-bar icons stay legible.
