...
- Add a secondary "Don't have an account? Create one on Bluesky" affordance on `LoginScreen`, below the primary submit button. Tapping it dispatches a new `LoginEvent.OpenSignup`; the VM responds by emitting the **existing** `LoginEffect.LaunchCustomTab(BLUESKY_SIGNUP_URL)`. No new effect variant — the screen's existing `LaunchedEffect` collector already opens `LaunchCustomTab` via `CustomTabsIntent`. Same pattern `:feature:feed:impl` uses for external embeds.
...
- `feature-login`: the "LoginViewModel drives beginLogin..." requirement currently mandates that failure populates `errorMessage` with the exception's `message` (or a generic fallback). That requirement is replaced by typed exception mapping. A new requirement covers the sign-up affordance — the `LoginEvent.OpenSignup` → `LoginEffect.LaunchCustomTab(BLUESKY_SIGNUP_URL)` wiring, plus the typed `LoginError` sum.
...
- **Backwards compatibility**: `LoginError.Failure(cause: String?)` is removed in favor of typed variants — internal to `:feature:login:impl`, no external module references it. `LoginEffect` is unchanged (`LaunchCustomTab` + `LoginSucceeded`); the sign-up CTA reuses the existing `LaunchCustomTab` variant, so the screen's `LaunchedEffect` `when` stays exhaustive without adding a new branch.
