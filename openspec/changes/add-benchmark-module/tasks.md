...
- [x] 1.2 Implement `AndroidBenchmarkConventionPlugin` at `build-logic/convention/src/main/kotlin/AndroidBenchmarkConventionPlugin.kt`. Apply `com.android.test`, `org.jetbrains.kotlin.android`, `androidx.benchmark`, `androidx.baselineprofile` (producer side). Configure compileSdk = 37, minSdk = 26, JVM 17 toolchain, `targetProjectPath = ":app"`, `experimentalProperties["android.experimental.self-instrumenting"] = true`, and the `benchmark` build type with `matchingFallbacks = listOf("release")`.
