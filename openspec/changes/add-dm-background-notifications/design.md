...
`getLog` returns an ordered, cursored event log; it's the right primitive for "what's new since last time." Persist one cursor per account in `:core:preferences` (DataStore). On each **backgrounded** run: fetch from the stored cursor, keep only **inbound** message-create events (sender ≠ viewer) **whose conversation is still unread server-side** (see read-state filter below), post per-convo notifications, then persist the advanced cursor **after** the post attempt (at-least-once; the client/notification id dedups a re-post). `listConvos` (v1's source) gives counts but not per-message identity, so it's wrong for per-message notifications.

**Read-state filter (foreground-read coordination).** The `getLog` cursor is a single global position across *all* convos, so it cannot be selectively "bumped past" one convo the user read while leaving another convo's interleaved messages pending — a per-convo manual cursor bump is not expressible. Instead, the notify decision is gated on **server-side read state**: an inbound event is notified only if its convo is still unread when the poll runs. When the user opens a conversation in the foreground, v1 already calls `chat.bsky.convo.updateRead` (`nubecita-1fy.18`), which clears that convo's server `unreadCount`; the next background poll re-reads the log and the now-read convo's events are filtered out. This makes "don't re-notify a message the user already read in the foreground" fall out of server state rather than fragile cursor arithmetic, and it pairs with D5's "don't advance the cursor on foreground-suppress."
...
### D5. Foreground suppression (no cursor advance)
While the app is foregrounded, v1's in-app surface already reflects unread, so the worker MUST NOT post. The worker checks an app-foreground signal (`ProcessLifecycleOwner` current state, read at run time) and, when foregrounded, **skips posting AND does not advance the cursor**. Advancing on a foreground-suppressed run would permanently drop those messages: the app can be foregrounded while the user is on a non-Chats screen (e.g. the home feed) and never sees the unread badge — if the cursor moved past the events, a later background poll would start after them and never notify. By holding the cursor, the messages are re-evaluated on the next background run, where the D3 read-state filter decides their fate: still unread → notify; read in the meantime (the user did open the thread → `updateRead`) → filtered out. So the cursor only advances when the worker actually handles events (a background run), never merely because a poll coincided with the foreground. WorkManager rarely runs in the foreground anyway; this is the correctness guard for the overlap window.
...
`enqueueUniquePeriodicWork(UNIQUE_NAME, ExistingPeriodicWorkPolicy.UPDATE, request)` on (sign-in ∧ opt-in); `cancelUniqueWork` on logout or opt-out. Driven from a production-flavor `AppInitializer` + reactions to `SessionStateProvider` / the preference flow (bench stays inert). `UPDATE` (not `KEEP`) so a later change to the request spec — interval, constraints — applies to existing installs in place without resetting the running schedule; for the idempotent repeat call (same spec, every session flip) it's effectively a no-op. The scheduler seam (`DmWorkScheduler`) is `suspend` and runs the WorkManager calls on `IoDispatcher`, since the reactive collector lives on the `Dispatchers.Default` CPU pool.
