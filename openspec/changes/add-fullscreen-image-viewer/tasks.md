...
- [ ] 1.2 Add `feature/mediaviewer/api/src/main/kotlin/net/kikin/nubecita/feature/mediaviewer/api/MediaViewer.kt` containing exactly:
  ```kotlin
  @Serializable
...
- [ ] 1.3 Register the new module in `settings.gradle.kts` (insert under the existing `:feature:postdetail:api` entry).
- [ ] 1.4 Run `./gradlew :feature:mediaviewer:api:compileDebugKotlin` clean.

## 2. New module: `:feature:mediaviewer:impl`

- [ ] 2.1 Create `feature/mediaviewer/impl/build.gradle.kts` applying the `nubecita.android.feature` convention plugin. Namespace `net.kikin.nubecita.feature.mediaviewer.impl`.
- [ ] 2.2 Declare deps: `api(project(":feature:mediaviewer:api"))`, `implementation(project(":designsystem"))`, `implementation(project(":data:models"))`, `implementation(project(":core:common"))`, `implementation(project(":core:posts"))`, `implementation(libs.telephoto.zoomable.image.coil3)`, `implementation(libs.coil.compose)`, plus the standard hilt + compose deps the convention plugin already wires.
- [ ] 2.3 Register the module in `settings.gradle.kts`.
- [ ] 2.4 Add `feature/mediaviewer/impl/src/main/kotlin/.../MediaViewerContract.kt` with `MediaViewerState`, `MediaViewerEvent`, `MediaViewerEffect`, `MediaViewerLoadStatus` per the design's Decision 3 sealed-sum shape. `MediaViewerEvent` covers: `OnPageChanged(index: Int)`, `OnTapImage`, `OnAltBadgeClick`, `OnAltSheetDismiss`, `OnDismissRequest` (back / close / swipe-down), `OnRetry`. `MediaViewerEffect` covers: `Dismiss`, `ShowError(message: UiText)`.
- [ ] 2.5 Add `feature/mediaviewer/impl/src/main/kotlin/.../MediaViewerViewModel.kt` extending `MviViewModel<MediaViewerState, MediaViewerEvent, MediaViewerEffect>`. Assisted-injected with `MediaViewerRoute`; constructor-injected with `:core:posts`'s `PostRepository`. `init` launches `postRepository.getPost(route.postUri)`. Read the resulting `PostUi.embed`: when it is `EmbedUi.Images`, project to `ImmutableList<ImageUi>`; when it is any other variant (defensive — viewer was opened on a non-image post), surface a `MediaViewerLoadStatus.Error(UiText.literal("This post has no images"))`. Failure → `Error(UiText.from(throwable))`.
- [ ] 2.6 Add `feature/mediaviewer/impl/src/main/kotlin/.../FullsizeUrl.kt` with the internal `ImageUi.fullsizeUrl(): String` helper per design Decision 6. Plain Kotlin file; no Compose / Hilt deps.
- [ ] 2.7 Add `feature/mediaviewer/impl/src/main/kotlin/.../MediaViewerScreen.kt`:
...
- [ ] 2.8 Add `feature/mediaviewer/impl/src/main/kotlin/.../di/MediaViewerNavigationModule.kt` providing the `@Provides @IntoSet @MainShell EntryProviderInstaller` mirroring `PostDetailNavigationModule` (assisted-inject ViewModel via `hiltViewModel<MediaViewerViewModel, MediaViewerViewModel.Factory>(creationCallback = { it.create(route) })`; `onDismiss = { LocalMainShellNavState.current.removeLast() }`).
- [ ] 2.9 Run `./gradlew :feature:mediaviewer:impl:compileDebugKotlin :feature:mediaviewer:impl:assembleDebug` clean.

## 3. Wire `:app` to pick up the entry

- [ ] 3.1 Add `implementation(project(":feature:mediaviewer:impl"))` to `app/build.gradle.kts` so Hilt's `@MainShell` multibinding includes the new entry. (Per the convention in `CLAUDE.md` / `build-logic/README.md`, only the `:impl` dep is needed; transitive `:api` flows from there.)
- [ ] 3.2 Run `./gradlew :app:assembleDebug` clean — verifies the multibinding compiles without conflict.

## 4. Modify `:feature:postdetail:impl`: hoist callback, remove Snackbar fallback

- [ ] 4.1 Add `implementation(project(":feature:mediaviewer:api"))` to `feature/postdetail/impl/build.gradle.kts`.
- [ ] 4.2 In `PostDetailScreen.kt`, add a hoisted parameter `onNavigateToMediaViewer: (postUri: String, imageIndex: Int) -> Unit` matching the existing `onNavigateToPost` / `onNavigateToAuthor` shape. Default to `{}` only if other navigation callbacks have defaults; otherwise no default (consistent with the surrounding hoisted-callback shape).
- [ ] 4.3 Replace the `is PostDetailEffect.NavigateToMediaViewer ->` branch in `PostDetailScreen.kt`'s effect collector. Old behavior (Timber log + `snackbarHostState.showSnackbar(mediaViewerComingSoonMessage)`) becomes `onNavigateToMediaViewer(effect.postUri, effect.imageIndex)`. Remove the `mediaViewerComingSoonMessage` local.
- [ ] 4.4 Delete `R.string.postdetail_snackbar_media_viewer_coming_soon` from `feature/postdetail/impl/src/main/res/values/strings.xml`. Run `./gradlew :feature:postdetail:impl:lint` to verify nothing references the removed string.
- [ ] 4.5 In `PostDetailNavigationModule.kt`, wire the new callback as `onNavigateToMediaViewer = { uri, index -> navState.add(MediaViewerRoute(postUri = uri, imageIndex = index)) }`.
...
- [ ] 5.2 Add `feature/mediaviewer/impl/src/test/kotlin/.../FullsizeUrlTest.kt`:
...
- [ ] 6.0 `./gradlew :core:posts:testDebugUnitTest :core:posts:assembleDebug` clean.
- [ ] 6.1 `./gradlew :feature:mediaviewer:api:testDebugUnitTest :feature:mediaviewer:api:assembleDebug` clean.
...
- [ ] 6.3 `./gradlew :feature:postdetail:impl:testDebugUnitTest :feature:postdetail:impl:validateDebugScreenshotTest` clean — regression contract for the Snackbar removal.
- [ ] 6.4 `./gradlew :app:assembleDebug spotlessCheck lint` clean.
...
- [ ] 6.7 Grep for the removed strings to confirm cleanup: `grep -rn "media_viewer_coming_soon\|nubecita-e02 -" .` should return zero hits in committed source. (The bd id reference in PR descriptions / commit footers is fine.)
