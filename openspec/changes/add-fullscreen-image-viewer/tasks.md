...
- [x] 6.8 PR description references this openspec change name (`add-fullscreen-image-viewer`), the bd id (`Closes: nubecita-e02`), and links the m28.5.2 design doc's Decision 4 (the placeholder this change retires).
- [x] 6.9 File a follow-up bd issue for "Feed PostCard image taps open the media viewer" — the deliberately-out-of-scope companion change. Cross-reference the issue id in this PR's description.
