...
- [ ] 2.1 Implement `BoldHeroGradient(banner: String?, avatarHue: Int, modifier: Modifier, content: @Composable () -> Unit)` in `:designsystem/src/main/kotlin/.../hero/BoldHeroGradient.kt`. `banner` is a nullable URL string (same shape as `app.bsky.actor.defs#profileViewDetailed.banner` and Coil's `model: Any?` in `NubecitaAsyncImage`). Decode the banner via Coil's `ImageLoader.execute()` on `Dispatchers.Default`; pass the bitmap to `Palette.from(...).generate()`; cache the result in a private LRU keyed on the banner URL; expose state as a `remember`'d gradient that swaps from the avatarHue fallback to the palette-derived gradient when extraction completes
