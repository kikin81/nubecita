...
- [ ] 3.1 Add `suspend fun refresh(feedKey: FeedKey): Result<Unit>` to `FeedRepository` — extract the mediator's REFRESH (fetch page 1 → transactional clear-partition + insert + cursor) into a shared repository method so the worker can refresh a partition WITHOUT a `Pager`/UI; have `FeedRemoteMediator`'s REFRESH delegate to it (no behavior change). Unit-test: `refresh` success writes page 1 + cursor; fetch failure returns `Result.failure` and leaves the cache intact.

## 4. Refresh runner (the logic + foreground guard)
- [ ] 4.1 `WidgetRefreshRunner` (plain injectable, mirrors `DmPollRunner`): gates in order — signed-out → `Outcome.SUCCESS` (no-op); `AppForegroundSignal.isForegrounded()` → `Outcome.SUCCESS` (no write, D-B4). Then for each MVP `FeedKey` (Following via `FeedKey.following(did)` + fixed Discover) call `repository.refresh(feedKey)`; any failure → `Outcome.RETRY`. On success, `repository.trimToCap(feedKey)` (off-scroll eviction). After all feeds refresh, call `WidgetUpdater.updateFeedWidgets()`. Return `Outcome.SUCCESS`.
