 ...
 *
 * Extracted as an interface so [DefaultPostingRepository] tests can
 * inject a fixed locale instead of reading the JVM's
 * `Locale.getDefault()` (which is per-process global mutable state).
 */
