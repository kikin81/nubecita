...
- **Repo is a singleton.** `DefaultPinnedFeedsRepository` is `@Singleton` (`FeedsModule.kt:17`), so an instance-level commit token is genuinely app-wide and shared with the `refresh()` caller.
...
1. **Trigger — `onCleared()`, only if dirty.** The single commit trigger is `ViewModel.onCleared()` (fires once when the screen is popped; retained across rotation, so no premature/duplicate commit). Guard: commit only if `currentOrder != seededOrder`. Skipping a no-op `putPreferences` on every visit protects the battery (a stated project priority). **Not** `DisposableEffect.onDispose` — that fires on config changes too.
2. **App-scoped execution.** `onCleared()` calls `repository.reorderPinnedFeeds(order)`, which launches on the injected `@ApplicationScope` scope — it must outlive the cancelled `viewModelScope`, or the `putPreferences` call dies in flight.
...
- **Repository tests** (`:core:feeds` JVM): `reorderPinnedFeeds` produces the correct `SavedFeedsPrefV2.items` array (pinned reordered + unpinned preserved + foreign prefs intact); `refresh()` skips reconciliation while `reorderPending`; Mutex serializes concurrent commits.
