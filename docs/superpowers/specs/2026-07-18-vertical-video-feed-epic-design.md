...
- **D5 — Decoder budget via handoff, released on `ON_STOP` (not just exit).** Entering the vertical feed pauses/releases `SharedVideoPlayer` (frees its decoder); the pool uses ≤2. The pool releases its players when the screen goes to **background (`Lifecycle.ON_STOP`)**, not only when the route is popped — otherwise navigating *forward* (feed → profile / post-detail) leaves the vertical feed in the back stack holding its 2 decoders while the new screen's `SharedVideoPlayer` grabs a 3rd → exhaustion on devices capped at 2. On return (`ON_START`), the pool re-prepares the active (and next) video. Respects device decoder limits (low-end phones cap at 1–2 hardware decoders).
...
- **[Prewarm vs prefetch races]** → gate `prepare()` on prefetch completion (Reddit `PriorityTaskManager`) to avoid Media3 `PlaybackException.ERROR_CODE_FAILED_RUNTIME_CHECK` (code 1004) / stalls.
