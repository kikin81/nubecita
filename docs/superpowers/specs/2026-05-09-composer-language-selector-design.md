...
- **Mirror Bluesky's `bsky-app` `LANGUAGES` constant verbatim** *(picked)*: 184 BCP-47 tags Bluesky exposes via the `code2`-tagged subset of their `LANGUAGES` array (their picker UI at `src/components/dialogs/LanguageSelectDialog.tsx` filters with `LANGUAGES.filter(lang => !!lang.code2)`). Users transitioning from the official client see the same options. Sync cost is one drive-by edit when their list changes (rare in practice). Stored in `:core:posting/.../BlueskyLanguageTags.kt` as a `val`-level constant — posting-domain data, not UI. The earlier "~75" estimate in this doc was based on an outdated read of `APP_LANGUAGES` (the app-translation-language list, ~41 entries) rather than the actually-selectable post-language set.
...
|---|---|---|
| **Compact** (phone) | `ModalBottomSheet { LanguagePickerContent(...) }` | Bottom sheets are the standard Compose pattern for mobile pickers — easy thumb reach, native scrolling for the 184-row list (search filter narrows quickly when needed). |
...
| **Search** | Top-of-picker `TextField`. Filter applies to BOTH the localized display name AND the BCP-47 tag, so `"en"` finds English and `"anglais"` finds English when the device is French. 184 entries scrolls but is long enough that typing 2-3 chars is meaningfully faster than thumb-scrolling — standard picker UX. |
...
- **Determinism across CI environments**: every screenshot fixture pins `deviceLocaleTag = "en"` on the VM (or the screenshot-host scaffold) so display-name rendering uses a known locale regardless of where the test runs. Without this pin, Layoutlib defaults to whatever `Locale.getDefault()` reports on the runner, and a CI machine configured for `fr-FR` would render `"Anglais"` while a developer on `en-US` rendered `"English"` — the diff against the committed baseline PNGs would fail spuriously. The `LocaleProvider` abstraction from `wtq.12` makes this pinning trivial: the screenshot host constructs the VM with `fixedLocaleProvider("en")`, mirroring the unit-test pattern already in `DefaultPostingRepositoryTest`.
