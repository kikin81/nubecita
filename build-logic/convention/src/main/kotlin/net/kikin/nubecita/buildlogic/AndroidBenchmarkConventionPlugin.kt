 ...
 *   installed target).
 * - `org.jetbrains.kotlin.android` — Kotlin on the test sources.
 ...
 *   profiles. The actual `BaselineProfileGenerator` test that produces
 *   a profile is a follow-up ticket (`nubecita-crmi.2`); landing the
 *   producer wiring here keeps that change scoped to "add the generator
 *   test" rather than "add the generator AND rewire build-logic".
 * - `com.squareup.sort-dependencies` — consistency with every other
 *   Android module in the repo.
 *
 * Configures compileSdk = 37 / minSdk = 28 (matching
 * [configureKotlinAndroid]), Java 17 source/target, JVM 17 toolchain,
 * `targetProjectPath = ":app"`, a `benchmark` build type with
 * `matchingFallbacks = listOf("release")`, and the
 ...
 * under `src/main/` can `@RunWith(AndroidJUnit4)` directly (the `com.android.test`
 * module type's main sources ARE the test sources — there's no separate
